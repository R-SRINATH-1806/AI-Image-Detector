...
def render_gpu_glsl_hologram(img_b64, depth_b64):
    ...
    <body>
        <div id="canvas-container">
            <div class="hud-overlay">👁️ MONVISION HOLOGRAM // MARK-85 TACTICAL PROJECTION</div>
            <div class="controls-panel">
                ...
                <div class="control-group">
                    ...
                    <button class="hud-btn" onclick="setPalette(0)">CYAN HUD</button>
                    <button class="hud-btn" onclick="setPalette(1)">MARK-85 GOLD</button>
                    <button class="hud-btn" onclick="setPalette(2)">RED ALERT</button>
                </div>
            </div>
        </div>
        <script>
            ...
            function setPalette(mode) { uniforms.uColorMode.value = mode; }
            function createTexture(b64) {
            ...
            const particleVS = `
                ...
                void main() {
                    ...
                    gl_PointSize = (uPointSize / -mvPos.z) * (1.0 + depth * 0.5);
            ...
            const particleFS = `
                ...
                void main() {
                    ...
                    vec3 col;
                    if (uColorMode == 0) col = mix(vec3(0.0, 0.4, 0.9), vec3(0.0, 1.0, 0.8), vDepth) * (0.5 + luma * 0.9);
                    else if (uColorMode == 1) col = mix(vec3(0.9, 0.3, 0.0), vec3(1.0, 0.8, 0.1), vDepth) * (0.5 + luma * 0.9);
                    else if (uColorMode == 2) col = mix(vec3(0.8, 0.0, 0.2), vec3(1.0, 0.2, 0.4), vDepth) * (0.5 + luma * 0.9);
                    else col = texColor.rgb;
            ...
            function init() {
                ...
                const grid = new THREE.GridHelper(9, 28, 0x00f3ff, 0x061428);
                grid.position.y = -0.01;
                scene.add(grid);

                uniforms.uColorMap.value = createTexture('__IMG_B64__');
                uniforms.uDepthMap.value = createTexture('__DEPTH_B64__');

                const geo = new THREE.PlaneBufferGeometry(3.2, 3.2, 260, 260);
                const mat = new THREE.ShaderMaterial({ uniforms: uniforms, vertexShader: particleVS, fragmentShader: particleFS, transparent: true });
                const pts = new THREE.Points(geo, mat);
                pts.position.y = 1.5;
                scene.add(pts);

                document.getElementById('sliderExtrude').oninput = (e) => uniforms.uExtrusionHeight.value = parseFloat(e.target.value);
                document.getElementById('sliderPSize').oninput = (e) => uniforms.uPointSize.value = parseFloat(e.target.value);
    ...
    """.replace("__IMG_B64__", img_b64).replace("__DEPTH_B64__", depth_b64)
    components.html(html_template, height=640)


# ---------------------------------------------------------
# 5. Dashboard State & Controls
# ---------------------------------------------------------
if "jarvis_booted" not in st.session_state:
    st.session_state.jarvis_booted = False
...
<div class="hud-banner">
    <div class="hud-title">MONVISION v11.0</div>
    <div class="hud-subtitle">// MARK-85 ADVANCED FORENSICS & HOLOGRAM CENTER</div>
    <div>
        <span class="hud-badge"><span class="pulse-dot"></span> J.A.R.V.I.S. VOICE & FORENSICS ONLINE</span>
    </div>
</div>
""",
    unsafe_allow_html=True,
...
with st.sidebar:
    st.markdown(
        """<h3 style="font-family: 'Orbitron', sans-serif; color: #00f3ff; font-size: 1.1rem;">⚙️ J.A.R.V.I.S. SPEECH SETTINGS</h3>""",
        unsafe_allow_html=True,
    )
    voice_speed = st.select_slider(
        "Speech Speed",
        options=["Slow", "Normal", "Fast", "Tactical Rapid"],
        value="Normal",
    )
    speed_map = {
        "Slow": "-10%",
    ...
    st.markdown(
        """<h3 style="font-family: 'Orbitron', sans-serif; color: #00f3ff; font-size: 1.1rem;">🛡️ ACTIVE MODULES</h3>""",
        unsafe_allow_html=True,
    )
    st.caption("• ViT Neural Patch Classifier")
    st.caption("• Chromatic Aberration Fringe Analysis")
    st.caption("• EXIF & PNG Metadata Inspector")
    st.caption("• 3D Fourier Spectral Topography")
    st.caption("• Intel DPT-MiDaS Depth Engine")

if not st.session_state.jarvis_booted:
    st.markdown(
        """
    <div style="text-align: center; padding: 40px 20px; background: rgba(2, 6, 16, 0.85); border: 2px solid #00f3ff; border-radius: 12px; margin: 20px 0;">
        <h2 style="font-family: 'Orbitron', sans-serif; color: #00f3ff;">J.A.R.V.I.S. FORENSICS OFFLINE</h2>
        <p style="color: #94a3b8; font-family: 'JetBrains Mono', monospace;">Click below to initialize Mark-85 audio, hologram, and deep forensic pipelines.</p>
    </div>
    """,
        unsafe_allow_html=True,
    )

    if st.button("⚡ POWER ON J.A.R.V.I.S. MARK-85 SYSTEMS", use_container_width=True):
        st.session_state.jarvis_booted = True
        st.session_state.speech_queue = (
            "Good morning, sir. MonVision Mark 85 online. Systems fully operational. Upload an image for deep forensic verification."
        )
        st.rerun()
    ...
    with col_input:
        uploaded_file = st.file_uploader(
            "UPLOAD FORENSIC IMAGE TARGET", type=["jpg", "png", "jpeg", "webp"]
        )
        paste_data = paste_image_button("📋 PASTE FROM CLIPBOARD")
    ...
    if uploaded_file is not None:
        ...
        image_id = f"pasted_{image_pil.size[0]}x{image_pil.size[1]}"

    # Small Target Preview Card
    if image_pil is not None:
        with col_preview:
            st.markdown(
                """<div style="font-family: 'Orbitron', sans-serif; color: #00f3ff; font-size: 0.8rem; margin-bottom: 5px;">📸 SOURCE PREVIEW</div>""",
                unsafe_allow_html=True,
            )
            st.image(image_pil, width=180)

    if image_pil is None:
        st.info(
            "🔊 **J.A.R.V.I.S.:** *'Systems ready, sir. Awaiting image upload or clipboard data to commence analysis.'*"
        )
    else:
        st.divider()

        is_new_image = st.session_state.analyzed_image_id != image_id

        with st.spinner(
            "🤖 J.A.R.V.I.S. EXECUTING MULTI-SPECTRAL NEURAL FORENSICS..."
        ):
        ...
        if fake_score > real_score:
            verdict_speech = (
                f"Sir, analysis complete. I have classified this image as A I Generated with {fake_score:.1f} percent confidence. "
                f"Here is how I detected it: {' '.join(spoken_reasons)}"
            )
        else:
            verdict_speech = (
                f"Sir, analysis complete. I have determined this image is Authentic with {real_score:.1f} percent confidence. "
                f"Here is how I verified it: {' '.join(spoken_reasons)}"
            )

        if is_new_image:
            st.session_state.speech_queue = verdict_speech
            st.session_state.analyzed_image_id = image_id
        ...
        with col_v1:
            ...
            m1, m2 = st.columns(2)
            m1.metric("FFT Lattice Energy", f"{freq_ratio:.1f}%")
            m2.metric("ELA Noise Variance", f"{ela_std:.2f}")

            m3, m4 = st.columns(2)
            m3.metric("Lens Fringe Index", f"{fringe_val:.2f}")
            m4.metric("Sharpness Variance", f"{lap_var:.1f}")

        with col_v2:
            render_cyber_header("🧠 EXPLAINABLE AI (XAI) EVIDENCE LOG")
        ...
        # Multi-View Tabs
        tab_holo, tab_2d, tab_3d, tab_exif, tab_report = st.tabs(
            [
                "⚡ TACTICAL HOLOGRAM",
                "🔬 2D SPECTRAL MAPS",
                "🌐 3D FREQUENCY TOPOGRAPHY",
                "📋 EXIF METADATA",
                "📄 DOSSIER REPORT",
            ]
        )
        ...
        with tab_holo:
            render_cyber_header("MARK-85 HOLOGRAM PROJECTION ENGINE")
            render_gpu_glsl_hologram(
                image_to_base64(image_pil), image_to_base64(depth_pil)
            )

        with tab_2d:
            render_cyber_header("FORENSIC ARTIFACT DETECTION MAPS")
            f1, f2, f3, f4 = st.columns(4)
            fft_img, fshift_abs = compute_fft(image_pil)
            ela_img = compute_ela(image_pil)
            fringe_img, _ = compute_chromatic_aberration(image_pil)

            with f1:
                st.image(
                    fft_img,
                    caption="2D Fourier Spectrum",
                    use_container_width=True,
                )
            with f2:
                st.image(
                    ela_img,
                    caption="Error Level Analysis",
                    use_container_width=True,
                )
            with f3:
                st.image(
                    compute_spatial_anomaly(image_pil),
                    caption="Laplacian Edge Anomaly",
                    use_container_width=True,
                )
            with f4:
                st.image(
                    fringe_img,
                    caption="Lens Fringe Dispersion",
                    use_container_width=True,
                )

        with tab_3d:
            render_cyber_header("3D SPECTRAL LATTICE TOPOGRAPHY")
            plotly_fft = generate_3d_fft_surface(fshift_abs)
            st.plotly_chart(plotly_fft, use_container_width=True)

        with tab_exif:
            render_cyber_header("METADATA & HARDWARE SIGNATURES")
            if exif_data:
                st.json(exif_data)
            else:
                st.warning(
                    "⚠️ No embedded EXIF camera header tags found. Note: Synthetic AI images (Midjourney, DALL-E) and social media platforms strip EXIF metadata."
                )

        with tab_report:
            render_cyber_header("FORENSIC BRIEFING DOSSIER")
            report_text = f"""# MONVISION FORENSIC INTELLIGENCE DOSSIER
**VERDICT:** {'AI GENERATED SYNTHETIC IMAGE' if fake_score > real_score else 'AUTHENTIC OPTICAL PHOTOGRAPH'}
**CONFIDENCE SCORE:** {max(fake_score, real_score):.2f}%

## TECHNICAL FORENSIC METRICS
- ViT AI Generator Confidence: {fake_score:.2f}%
- ViT Authentic Photo Confidence: {real_score:.2f}%
- Fourier High-Frequency Energy Ratio: {freq_ratio:.2f}%
- Error Level Analysis Noise Std Dev: {ela_std:.2f}
- Chromatic Aberration Dispersion Index: {fringe_val:.2f}
- Laplacian Edge Variance: {lap_var:.2f}

## EXPLAINABLE AI EVIDENCE BREAKDOWN
{chr(10).join(['- ' + r for r in xai_reasons])}
"""
            st.code(report_text, language="markdown")
            st.download_button(
                "📥 DOWNLOAD FORENSIC DOSSIER",
                data=report_text,
                file_name="MONVISION_FORENSIC_DOSSIER.md",
                mime="text/markdown",
            )

# Fire queued audio ONCE
if st.session_state.speech_queue:
    render_jarvis_speech(
        st.session_state.speech_queue, rate=speed_map[voice_speed]
    )
    st.session_state.speech_queue = None